...
import mdpy.forces as _forces

class LJ126Test(unittest.TestCase):
  """Test cases for `mdpy.forces.LJ126()`."""
  ...
  def setUp(self):
    ...
    self.box = _box.PBCBox(xdim=xdim, ydim=ydim, zdim=zdim, coordinates=self.cor)
    self.lj_sig = np.abs(np.random.randint(1, 10) * np.random.randn())
    self.lj_eps = np.abs(np.random.randint(1, 10) * np.random.randn())
    self.lj_126 = _forces.LJ126(sigma=self.lj_sig, epsilon=self.lj_eps)
  ...
  def test_get_energy(self) -> None:
    ...
    # test.
    ener = self.lj_126.get_energy(box=self.box)
  ...
  def test_get_forces(self) -> None:
    ...
    # test.
    ener = self.lj_126.get_energy(box=self.box)
    forc = self.lj_126.get_forces(box=self.box)
